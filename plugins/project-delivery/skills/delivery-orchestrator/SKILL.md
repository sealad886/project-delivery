...
4. Select exactly one canonical profile from `../.shared/route-profiles-v1.json` by semantic intent, lifecycle state, and authority—not by exact wording. Apply its required owners, declared conditional owners, precedence, re-entry, final-controller, forbidden-owner, evidence, artifact, and stop rules.
   ...
   - `workflow-decommission` remains an orchestrated composite: inventory active instructions, prompts, hooks/configuration, state, installed identities/versions, exact recovery sources, and legacy imperative artifacts → preserve rollback → disable one candidate at a time → start an independently resolved fresh task and run affected static/live smoke cases → re-enable on any routing or parity failure → uninstall only after observation gates and user-confirmed scope. Do not edit consumer repositories or plugin configuration without corresponding authority.
...
- Evaluate retrospective only when the selected profile declares it conditional or required. For a declared retrospective with no meaningful outcome yet, record `planned-future` with `future-pending` and omit it from `actual_route`. When outcome evidence is unknown, defer or block it with a linked structured gap. A direct retrospective profile makes `retrospective-improvement` required and never duplicates it as conditional. Mere code, PR, or change completion is not by itself an observed user, business, operational, failure, recovery, cancellation, or recurring-friction outcome.
