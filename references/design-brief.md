...
All skills share one operating model and template set. The orchestrator is instructed to resolve selected sibling skills from the installed bundle so global initial-list budgeting cannot silently erase required lifecycle ownership. Standard-library checkers validate metadata, internal links, semantic route policy, frozen blind receipts, names, and placeholders. A minimum runtime-closure simulation proves that the runtime subset is internally complete without claiming to reproduce Codex or marketplace packaging. Audit, research, smoke, and decommission records are versioned with the plugin.
